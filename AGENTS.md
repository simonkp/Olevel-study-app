...
- **Hub / parent:** `write-hub-tail-scripts.js` / **`write-parent-tail-scripts.js`** — same pattern.
...
3. **`subject-config.js`** — sets `window.SUBJECT_ID`, `SUBJECT_TITLE`, runs first-run setup, assigns **`window.__LEVELUP_SUBJECT_SETUP`**, then `LEVELUP_STUDENT_*` / `SUPABASE_*` from `localStorage`.
